...
plotItems <- list(
  list(key = "item1", name = "プロットタイプ", subMenuProps = list(
    items = list(
      list(key = 'plotBasic', name = 'プロット', onClick = JS("function() { Shiny.setInputValue('plot', 'plotBasic'); console.log('convertType'); }")),
      list(key = 'plotAgg', name = '集計', onClick = JS("function() { Shiny.setInputValue('plot', 'plotAgg'); }")),
      list(key = 'plotFund', name = '基本統計量', onClick = JS("function() { Shiny.setInputValue('plot', 'plotFund'); }")),
      list(key = 'plotImp', name = '重要度', onClick = JS("function() { Shiny.setInputValue('plot', 'plotImp'); }")),
      list(key = 'plotCorr', name = '変数相関', onClick = JS("function() { Shiny.setInputValue('plot', 'plotCorr'); }")),
      list(key = 'plotLabel', name = 'イベントラベル', onClick = JS("function() { Shiny.setInputValue('plot', 'plotLabel'); }"))
    )
  ))
)
...
SavePlotItem <- function(inputKey) {
  switch(inputKey,
         "plotBasic" = div(plotOutput("plot2",height = '100%'), style = "height: 40vh"),
         "plotAgg" = div(plotOutput("aggplot2",height = '100%'), style = "height: 40vh"),
         "plotFund" = DT::dataTableOutput('summary2'),
         "plotImp" = div(plotOutput("sigplot2",height = '100%'), style = "height: 40vh"),
         "plotCorr" = div(plotOutput("corrplot2",height = '100%'), style = "height: 40vh"),
         "plotLabel" = div(plotOutput("labelplot2",height = '100%'), style = "height: 40vh"),
         "plotClust" = div(plotOutput("clustering_plot2",height = '100%', DT::dataTableOutput("colinfo_for_clustering_result")), style = "height: 40vh"),
         div(plotOutput("plot2",height = '100%'), style = "height: 40vh")
  )
}

PlotItem <- function(inputKey) {
  switch(inputKey,
         "plotBasic" = div(plotOutput("plot",height = '100%'), style = "height: 40vh"),
         "plotAgg" = div(plotOutput("aggplot",height = '100%'), style = "height: 40vh"),
         "plotFund" = DT::dataTableOutput('summary'),
         "plotImp" = div(plotOutput("sigplot",height = '100%'), style = "height: 40vh"),
         "plotCorr" = div(plotOutput("corrplot",height = '100%'), style = "height: 40vh"),
         "plotLabel" = div(plotOutput("labelplot",height = '100%'), style = "height: 40vh"),
         "plotClust" = div(plotOutput("clustering_plot",height = '100%', DT::dataTableOutput("colinfo_for_clustering_result")), style = "height: 40vh"),
         div(plotOutput("plot",height = '100%'), style = "height: 40vh")
  )
}
  ...
  switch(inputKey,
         ...
         "clustering" = list(
           ...
           TagPicker(
             ...
             pickerSuggestionsProps = list(suggestionsHeaderText = '変数リスト', noResultsFoundText = 'データセットが読み込まれていません。'),
             itemLimit = 1,
             onChange = JS("function(selection) { Shiny.setInputValue('colinfo_for_clustering', JSON.stringify(selection)) }")
           ),
           Text(variant = "medium", "クラスタ数"),
           TextField.shinyInput("num_cluster"),
           Text(variant = "medium", "使用する距離関数"),
           Dropdown("clustering_type", options = options_clustering),
           Text(variant = "medium", "基準化"),
           Dropdown("c_normalize_type", options = options_normalize),
           br(),
           PrimaryButton.shinyInput("exec_clustering", text = "適用")
         ),
         "characterMerge" = list(
         ...
         "plotlyPlot" = list(
           ...
           uiOutput("conditionalcontrols"),
           br(),
           PrimaryButton.shinyInput("plotlyPlotButton", text = "適用")
         ),
         "aggPlot" = list(
         ...
         "labelPlot" = list(
           ...
           TextField.shinyInput("span", defaultValue = "0.05"),
           Text(variant = "medium", "Window Size"),
           TextField.shinyInput("window_size", defaultValue = "2"),
           Text(variant = "medium", "ラベルモード"),
           Dropdown.shinyInput("modetype", options = options_labelMode),
           br(),
           PrimaryButton.shinyInput("make_labelplot", text = "プロット"),
           br(),
           PrimaryButton.shinyInput("make_label", text = "ラベル生成"),
           Text(variant = "small", '*ラベル生成すると新しくカラムが追加されます')
         )
  )
}
    ...
    GridItem(
      ...
      Card(
        ...
        div(
          ...
          Text(variant = "large", 'コントロールパネル'),
          div(
            style="text-align:right;float:right;",
            undoHistoryUI("hist", back_text = "元に戻す", fwd_text = "やり直し")
          )
        ),
        filterMenu,
